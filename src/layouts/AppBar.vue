...
const avatarUrl = ref<string>('default_avatar.png');
...
const getProfile = async () => {
    try {
        ...
        showMessage("Failed to get profile");
    ...
    if (selectedUser) {
        try {
            ...
            if (response.status === 200) {
                ...
                showMessage(`失败: ${response.status}`);
            }
        } catch (error) {
            showMessage(`请求失败: ${error.response?.status}`);
        }
    } else {
        showMessage("请选择一个用户进行切换！");
    }
    selectedUsers.value = [];
};
